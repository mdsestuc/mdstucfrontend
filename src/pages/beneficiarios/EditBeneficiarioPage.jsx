...
export const EditBeneficiarioPage = () => {
    const { _id } = useParams();
    ...
    const { getBeneficiario, 
            ...
            editBeneficiario,
            getEstudios,
            getProgramasociales  } = useContext(BeneficiarioContext);


    const { formState, onInputChange, addProvisetForm, onResetForm, addLatyLon} = useForm({
        nombre: state?.beneficiario.nombre,
        apellido: state?.beneficiario.apellido,
        documento: state?.beneficiario.documento,
        idprovincia: state?.beneficiario.idprovincia,
        provincia: state?.beneficiario.provincia,
        iddepartamento: state?.beneficiario.iddepartamento,
        departamento: state?.beneficiario.departamento,
        idmunicipio: state?.beneficiario.idmunicipio,
        municipio: state?.beneficiario.municipio, 
        idlocalidad: state?.beneficiario.idlocalidad,
        localidad: state?.beneficiario.localidad,
        domicilio: state?.beneficiario.domicilio,
        numero: state?.beneficiario.numero,
        piso: state?.beneficiario.piso,
        cp: state?.beneficiario.cp,
        idsexo: state?.beneficiario.idsexo._id,
        genero: state?.beneficiario.idsexo.nombre,
        fechanacimiento:  moment(state?.beneficiario.fechanacimiento).format("YYYY-MM-DD"),
        nrocel1: state?.beneficiario.nrocel1,
        nrocel2: state?.beneficiario.nrocel2,
        latitud: state?.beneficiario.latitud,
        longitud: state?.beneficiario.longitud,
        idestudio: state?.beneficiario.idestudio._id,
        estudio: state?.beneficiario.idestudio.nombre,
        jubilacion: state?.beneficiario.jubilacion,
        pension: state?.beneficiario.pension,
    });
    ...
    let iniciarProvincia = {    
                                id: state?.beneficiario.idprovincia, 
                                nombre: state?.beneficiario.provincia, 
                                provincia: 'Provincia', 
    ...
    //programas sociales, inicializo
    const [checked, setChecked] = React.useState([]);
    const iniciaprogramas = () => { 
        let listprogram = state?.beneficiario.programasocialId.map((value) => value._id);
        setChecked(listprogram);
    }
    ...
    console.log(state.beneficiario.latitud, state.beneficiario.longitud,  "latitud y longitus del state")

    const [coordenadas, SetCoordenadas] = useState([ { lat: state.beneficiario.latitud, lon: state.beneficiario.longitud }]); 
    ...
    const beneficiarioEdit = async () => {
        ...
        //esta todo OK guardo las modificaciones del Beneficiario
        try { 
            let modificarBeneficiario = {
                id: _id,
            ...
            await editBeneficiario(modificarBeneficiario);
            alert('Se actualizo el Beneficiario con exito!');
    ...
    <Paper>
        <Box padding={3}>
            <Grid container spacing={3}>
                ...
                >
                    <Typography variant='h6' fontFamily={'fantasy'}>Modificar Beneficiario</Typography>
                ...
                <Grid item xs={12} md={12}>
                    {formState.latitud === 0 || formState.latitud === "0"
                    ?                     
                    <Button 
                        variant='contained' 
                        size='large' 
